...
class DocumentExtractor:
    ...
    def read_document(self, file_path: str, pdf_extract_method: str = "pypdf2") -> DocumentContent:
        ...
        Args:
            file_path: 文件路径或OSS URL
            pdf_extract_method: PDF提取方式，可选"pypdf2"或"mineru"
        """
        # =================================================================
        # 🔥 核心修复：安全提取文件后缀，完美兼容阿里云OSS无后缀URL
        # =================================================================
        clean_path = file_path.split('?')[0]
        file_extension = clean_path.split('.')[-1].lower()

        # 如果提取出来的后缀包含斜杠（例如 "com/unnamed_upload_file"）
        # 说明这是一个没有真实扩展名的 URL。
        # 此时如果调用方指定了使用 "mineru"，我们强制将其识别为 pdf！
        if "/" in file_extension and pdf_extract_method == "mineru":
            file_extension = 'pdf'
        # =================================================================
