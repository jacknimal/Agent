...
class MilvusStorage:
    ...
    def create_hybrid_retriever(self, **kwargs):
        ...
        try:
            ...
            # 1. 显式建立底层连接（解决 ConnectionNotExistException）
            if self.token:
            ...
            col.flush()  # 强制将内存数据落盘
            col.load()  # 强行将集合加载到内存（这是检索出数据的核心！）
            print(f"🔍 [DEBUG] 检索启动，集合 {self.collection_name} 当前库内实体总数: {col.num_entities}")
        except Exception as e:
            print(f"❌ [DEBUG] 获取集合统计失败: {str(e)}")
        # ================= 🔥 调试与强制加载代码 结束 🔥 =================
